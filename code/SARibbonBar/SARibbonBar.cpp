...
public:
    ...
    void setApplicationButton(QAbstractButton *btn)
    {
        ...
        if (btn) {
            ...
            MainClass->connect(btn, &QAbstractButton::clicked
                , MainClass, &SARibbonBar::applicationButtonClicked);
        }
...
{
    if (isContextCategoryVisible(context) || !context ) {
        return;
    }
...
void SARibbonBar::showMinimumModeButton(bool isShow)
{
    if (isShow) {
        ...
        if (nullptr == m_d->minimumCaterogyButton) {
			...
			action->setIcon(QIcon(isMinimumMode() ?
				":/image/resource/ArrowDown_white.png" : ":/image/resource/ArrowUp_white.png"));
...
void SARibbonBar::activeRightButtonGroup()
{
    if (nullptr == m_d->rightButtonGroup) {
        ...
        m_d->rightButtonGroup->setObjectName("rightButtonGroup");
    }
...
void SARibbonBar::paintWindowTitle(QPainter& painter, const QString& title, const QRect& titleRegion)
{
    ...
    auto f = painter.font();
    QFontMetrics fm(f);
    //QString afterTitle = QString("(%1)").arg(title.mid(title.indexOf('-') + 1));
    QString afterTitle = QString("(%1)").arg(title.right(title.size() - title.indexOf('-') - 1));
    QRect rec = fm.boundingRect(afterTitle);
    //图标大小 130 * 21, 图标和文字间距 5
    int startX = titleRegion.x() + (titleRegion.width() - rec.width()) * 0.5 - 130 - 5; 
    int startY = titleRegion.y() + (titleRegion.height() - 21) * 0.5;
	QPixmap mp(":/image/resource/CyberUniverse.png");
	painter.drawPixmap(startX, startY,130,21, mp);  
    painter.setPen(RibbonSubElementStyleOpt.titleTextColor);
    painter.drawText(titleRegion, Qt::AlignCenter, afterTitle);
    painter.restore();
}
