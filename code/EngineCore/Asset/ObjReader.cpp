...
bool ObjReader::ReadObj(const wchar_t * objFileName)
{
	...
	for (;;)
	{
		...
		else if (wstr == L"f")
		{
			...
			// 几何面
			//
			Geometry::VertexPosNormalTex vertex;
...
bool ObjReader::ReadMbo(const wchar_t * mboFileName)
{
	...
	for (UINT i = 0; i < parts; ++i)
	{
		...
		fin.read(reinterpret_cast<char*>(objParts[i].vertices.data()), vertexCount * sizeof(Geometry::VertexPosNormalTex));
...
bool ObjReader::WriteMbo(const wchar_t * mboFileName)
{
	...
	for (UINT i = 0; i < parts; ++i)
	{
		...
		if (vertexCount > 65535)
		{
			...
			// [顶点]32*顶点数 字节
			fout.write(reinterpret_cast<const char*>(objParts[i].vertices.data()), vertexCount * sizeof(Geometry::VertexPosNormalTex));
			// [索引]4*索引数 字节
...
void ObjReader::AddVertex(const Geometry::VertexPosNormalTex& vertex, DWORD vpi, DWORD vti, DWORD vni)
{
