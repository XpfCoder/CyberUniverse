...
{
	vertexStride = sizeof(Geometry::VertexPosNormalTex);
	...
	for (size_t i = 0; i < model.objParts.size(); ++i)
	{
		...
		vbd.ByteWidth = modelParts[i].vertexCount * (UINT)sizeof(Geometry::VertexPosNormalTex);
		...
		device->CreateBuffer(&vbd, &InitData, modelParts[i].vertexBuffer.ReleaseAndGetAddressOf());
		...
		// 新建索引缓冲区
		device->CreateBuffer(&ibd, &InitData, modelParts[i].indexBuffer.ReleaseAndGetAddressOf());
		...
		if (strD.size() > 4)
		{
			...
			{
				CreateDDSTextureFromFile(device, strD.c_str(), nullptr,
					modelParts[i].texDiffuse.GetAddressOf());
			}
			else
			{
				CreateWICTextureFromFile(device, strD.c_str(), nullptr,
					modelParts[i].texDiffuse.GetAddressOf());
			}
		}
...
void Model::SetMesh(ID3D11Device * device, const void * vertices, UINT vertexSize, UINT vertexCount, const void * indices, UINT indexCount, DXGI_FORMAT indexFormat)
{
	...
	device->CreateBuffer(&vbd, &InitData, modelParts[0].vertexBuffer.ReleaseAndGetAddressOf());
	...
	device->CreateBuffer(&ibd, &InitData, modelParts[0].indexBuffer.ReleaseAndGetAddressOf());
