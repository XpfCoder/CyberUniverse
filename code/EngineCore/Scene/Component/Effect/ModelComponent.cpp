...
{
	ModelComponent::ModelComponent(void** parameter) : IComponent(parameter)
	{
		...
		setUseTexOrColor(false);
	}

	ModelComponent::ModelComponent(ComPtr<ID3D11Device> pd3dDevice, ComPtr<ID3D11DeviceContext> pd3dImmediateContext) : IComponent(pd3dDevice, pd3dImmediateContext)
	{
		setComponetType("ModelComponent");
		...
		setUseTexOrColor(true, {0.4,0.6,0.9,1});
	}

	void ModelComponent::buildMesh()
	{
		m_pVertexBuffer.Reset();
		m_pIndexBuffer.Reset();

		const UINT vertexCount = 24;
		Geometry::VertexPosNormalTex vertices[vertexCount];
	...
	void ModelComponent::createVertexLayout(const ComPtr<ID3DBlob>& blob)
	{
	...
	void ModelComponent::bindPipeState()
	{
